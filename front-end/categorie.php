...
<head>
    ...
    <?php 
     ...
     $id=$_GET['id'];
     $query1="SELECT * FROM produit WHERE id_categorie='$id'";
     $res1=mysqli_query($con,$query1,MYSQLI_USE_RESULT);
     $produits = mysqli_fetch_all($res1); 
     include '../include/nav-front.php';
     $query="SELECT * FROM catergorie WHERE id='$id'";
     $res=mysqli_query($con,$query,MYSQLI_USE_RESULT);
     $categorie = mysqli_fetch_row($res);

    ?>

    <title><?php echo $categorie['1'] ?></title>
    
</head>
<body>
   <?php  ?>
          <div class="main">
              <h2><?php echo $categorie['1'] ?></h2>
          </div>
